 ...
 *
 * @description :: Server-side logic for managing Statisticshours
...
module.exports = {
  ...
  getStatisticsByHour: function(req, res) {
    ...
    fromDate = new Date(fromDate.getFullYear(), fromDate.getMonth(), fromDate.getDate(), fromDate.getHours(), 0, 0); // shift to soh
    let toDate = new Date(toInMills);
    toDate = new Date(toDate.getFullYear(), toDate.getMonth(), toDate.getDate(), toDate.getHours(), 59, 59); // shift to eoh
    ...
    StatisticsHour.find({
      ...
      if (!responseData) { return res.badRequest(generateResultService.fail('Could not find any StatisticsHour records')); }
